...
namespace NUnit.VisualStudio.TestAdapter.Navigation
{
    ...
    internal class FullSymbolReader : ISymbolReader 
    {
        ...
        private NavigationData GetSymbolNavigationData(IDiaSymbol symbol)
        {
            if (symbol == null)
                throw new ArgumentNullException(nameof(symbol));

            NavigationData navigationData = new NavigationData(null, int.MaxValue, int.MinValue);
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        ...
                        navigationData.FilePath = sourceFile.fileName;
                        navigationData.LineNumber = Math.Min(navigationData.LineNumber, (int)lineNumber.lineNumber);
        ...
        private void PopulateCacheForTypeAndMethodSymbols()
        {
            ...
            try
            {
                ...
                while (celtTypeSymbol == 1 && null != typeSymbol)
                {
                    ...
                    {
                        Debug.WriteLine($"Ignoring the exception while iterating method symbols:{ex} for type:{typeSymbol.name}");
                    }
                    finally
                    {
            ...
            {
                Debug.WriteLine($"Ignoring the exception while iterating type symbols:{ex}");
            }
            finally
            {
